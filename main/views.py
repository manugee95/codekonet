...
def profile_update(request):
    ...
    if request.method == 'POST':
        ...
        if pform.is_valid():
            ...
            new = user.first_name.title()
            messages.error(request, f'dear {new} your profile updated generated the following error: {pform.errors}')
...
def pay(request):
    if request.method == 'POST':
        ...
        cburl = 'http://54.225.56.119/callback' #payment thank you page 
        ref = str(uuid.uuid4()) #reference number required by pastack as an additional order number
        ...
        add_info = request.POST['add_info'] #collect from the template incase there is a change
